...
| `orbbec_camera` | Orbbec SDK ROS 2 wrapper (RGB-D camera support) |
...
| Library | Description |
|---------|-------------|
| `lib/librectrl.so` | Vendor camera control SDK (precompiled aarch64 binary) |
...
> **Note:** The `orbbec_camera` package is declared as a build dependency and must be present in the workspace. If you do not need RGB-D camera support, remove `orbbec_camera` from `package.xml` and the `GroupAction` block in `launch/camera.launch.py`.
...
┌──────────────────────────────────────────┐
│  RGB-D camera (external orbbec_camera)    │
...
|--------|---------|----------|
| `V4l2Driver` | Linux V4L2 (mmap) | V4L2 cameras, generic V4L2 cameras |
| `UsbDriver` | OpenCV `VideoCapture` | Standard USB cameras |
| RGB-D camera | External `orbbec_camera` node | Orbbec RGB-D camera |
...
v4l2_driver:
  ...
  use_rom_mode: true        # Enable ROM-based calibration (V4L2 cameras)
  port_info:
...
|-----------|---------|-------------|
| `port` | (required) | Port ID string used for device lookup via sysfs. V4L2 cameras use the USB port suffix (e.g., `2-001a`). Generic cameras match by device name substring. |
...
| `vendor` | `"novitec"` | Vendor handler selection. Use `"Generic"` for non-V4L2 cameras. |
...
The RGB-D camera is not configured via `camera_config.yaml`. It is enabled through launch arguments that include the `orbbec_camera` launch file. See the [Usage](#usage) section for launch arguments.
...
The RGB-D camera is always launched alongside the main camera node. To specify a particular device, pass the serial number or USB port:
...
| `gemini336l_serial_number` | `""` | RGB-D camera serial number filter |
| `gemini336l_usb_port` | `""` | RGB-D camera USB port filter |
...
For the RGB-D camera (launched via `orbbec_camera`), topics appear under the `/sensor/camera/gemini336l/front/` namespace following the Orbbec SDK topic structure.
...
- For V4L2 cameras, verify the port ID matches the sysfs entry:
  ```bash
...
- Check the camera bridge board firmware version in the log output.
